...
package com.audic.music.audicmusic.updater
...
import com.audic.music.audicmusic.updater.downloadmanager.UpdateDownloadWorker
import com.audic.music.audicmusic.updater.downloadmanager.DownloadNotificationManager
...
import com.audic.music.ui.component.ChangelogItem
import com.audic.music.ui.component.leadingItemShape
import com.audic.music.ui.component.middleItemShape
import com.audic.music.ui.component.endItemShape
import com.audic.music.ui.component.detachedItemShape
import com.audic.music.ui.component.parseMarkdown
import com.audic.music.ui.component.endItemShape
import com.audic.music.ui.component.detachedItemShape
import com.audic.music.ui.component.AnimatedActionButton
import com.audic.music.ui.component.ExpressiveIconButton
import com.audic.music.ui.component.ErrorSnackbar
...
sealed class AudicUpdateStatus {
    object Idle : AudicUpdateStatus()
    object Checking : AudicUpdateStatus()
    ...
    data class NoUpdate(val version: String) : AudicUpdateStatus()
    data class Error(val message: String) : AudicUpdateStatus()
}
...
fun UpdateScreen(navController: NavHostController) {
    ...
    var status by remember { mutableStateOf<AudicUpdateStatus>(AudicUpdateStatus.NoUpdate(BuildConfig.VERSION_NAME)) }
    ...
    fun triggerUpdateCheck() {
        status = AudicUpdateStatus.Checking
        scope.launch {
            ...
            checkForUpdate(
                ...
                onSuccess = { tag, isAvailable, changelog, size, date, description, imageUrl, apkUrl ->
                    ...
                    status = if (isAvailable) {
                        ...
                        AudicUpdateStatus.NoUpdate(tag)
                    }
                },
                onError = {
                    status = AudicUpdateStatus.Error(context.getString(R.string.cant_check_updates))
                }
            )
        }
    }
    ...
    Scaffold(
        topBar = {
            LargeTopAppBar(
                title = {
                    val titleText = if (status is AudicUpdateStatus.Available) {
                        buildAnnotatedString {
                            ...
                            withStyle(
                                ...
                                append((status as AudicUpdateStatus.Available).version)
                            }
                        }
                    } else {
            ...
            Surface(
                ...
                Column(
                    ...
                    when (val currentStatus = status) {
                        is AudicUpdateStatus.Idle, is AudicUpdateStatus.Checking, is AudicUpdateStatus.NoUpdate, is AudicUpdateStatus.Error -> {
                            AnimatedActionButton(
                                text = stringResource(R.string.check_for_update),
                                onClick = { triggerUpdateCheck() },
                                enabled = currentStatus !is AudicUpdateStatus.Checking && !isDownloading,
                                modifier = Modifier.fillMaxWidth()
                            )
                        }

                        is AudicUpdateStatus.Available -> {
                            Row(
                                modifier = Modifier.fillMaxWidth(),
        ...
        Box(
            ...
            LazyColumn(
                ...
                item {
                    val contentModifier = if (status is AudicUpdateStatus.Available) {
                    ...
                    Box(
                        ...
                        when (val currentStatus = status) {
                            is AudicUpdateStatus.Checking -> {
                                androidx.compose.material3.ContainedLoadingIndicator(
                                    modifier = Modifier.size(64.dp)
                                )
                            }

                            is AudicUpdateStatus.NoUpdate -> {
                                Column(
                                    horizontalAlignment = Alignment.CenterHorizontally
                                ) {
                                    Icon(
