package com.audic.music.audicmusic.updater.downloadmanager
...
class UpdateDownloadWorker(private val context: Context, workerParams: WorkerParameters) :
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val downloadFile = if (isZip) File(downloadDir, "audic_temp.zip") else File(downloadDir, "audicmusic.apk")
            ...
            val finalFile = if (isZip) {
                val targetApkFile = File(downloadDir, "audicmusic.apk")
                var extracted = false
                try {
