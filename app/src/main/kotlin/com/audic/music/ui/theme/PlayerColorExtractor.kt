...
package com.audic.music.ui.theme
...
object PlayerColorExtractor {
    ...
    suspend fun extractGradientColors(
        ...
        val primaryColor = if (bestSwatch != null) {
            ...
            if (isColorVibrant(bestColor)) {
                enhanceColorAudicness(bestColor, 1.3f)
            } else {
                
                enhanceColorAudicness(fallbackDominant, 1.1f)
            }
        } else {
            enhanceColorAudicness(fallbackDominant, 1.1f)
        }
    ...
    private fun enhanceColorAudicness(color: Color, saturationFactor: Float = 1.4f): Color {
        val argb = color.toArgb()
        val hsv = FloatArray(3)
        android.graphics.Color.colorToHSV(argb, hsv)
